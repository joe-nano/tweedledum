...
#pragma region Constructors
	BitMatrixRM(size_type num_rows) : storage_(num_rows) {}

	BitMatrixRM(size_type num_rows, size_type num_columns)
	    : storage_(num_rows, num_columns)
	{}

	template<typename ValueType>
	BitMatrixRM(size_type num_columns, std::vector<ValueType> const& rows)
...
#pragma region Modifiers
	...
	auto permute_rows(std::vector<uint32_t> const& rows) const
	{
		...
		BitMatrixRM permuted_matrix(num_columns());
		for (auto row : rows) {
