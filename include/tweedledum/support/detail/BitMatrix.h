...
struct BitMatrix {
	static_assert(std::is_integral<WordType>::value, "Integral type "
	                                                 "required.");
	static_assert(!std::is_same<WordType, bool>::value, "Not bool though "
	                                                    "(:");

	using word_type = WordType;
	using line_type = DynamicBitset<word_type>;
	using size_type = std::size_t;
	using container_type = std::vector<line_type>;

#pragma region Constructors
	BitMatrix(size_type num_bits_per_line)
	    : num_bits_per_line_(num_bits_per_line), lines_()
	{}

	BitMatrix(size_type num_bits_per_line, size_type num_lines)
	...
	BitMatrix(
	    size_type num_bits_per_line, std::vector<ValueType> const& words)
...
#pragma region Modifiers
	constexpr void push_back_line(DynamicBitset<WordType> const& line)
	{
	...
	// I wonder if it wouldn't be better to return a new transposed version
	// of this matrix
	void transpose()
	{
		// Take a faster path
		...
		assert(0
		       && "Transpose method not implemented for the general "
		          "case");
		std::abort();
	}
#pragma endregion
...
private:
	template<typename ValueType>
	void init_from_value_vector(
	    size_type num_rows, std::vector<ValueType> const& lines_values)
	{
		static_assert(std::is_integral<ValueType>::value, "Integral "
		                                                  "type "
		                                                  "required.");
