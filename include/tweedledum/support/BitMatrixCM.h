...
// TODO: Change this to a policy!
/*! \brief Column-major matrix of bits */
template<typename WordType = uint32_t>
class BitMatrixCM {
public:
	using word_type = WordType;
	using row_type = DynamicBitset<WordType>;
	using column_type = DynamicBitset<WordType>;
	using size_type = std::size_t;
	using container_type = detail::BitMatrix<WordType>;

#pragma region Constructors
	BitMatrixCM(size_type num_rows) : storage_(num_rows) {}

	BitMatrixCM(size_type num_rows, size_type num_columns)
	    : storage_(num_rows, num_columns)
	{}

	template<typename ValueType>
	BitMatrixCM(size_type num_rows, std::vector<ValueType> const& columns)
...
#pragma region Element access
	...
	constexpr auto row(
	    std::vector<uint32_t> const& columns, size_type row_index) const
	{
		row_type value(columns.size(), 0);
		for (size_type column_index = 0; column_index < columns.size();
		     ++column_index) {
			value[column_index]
			    = column(columns[column_index])[row_index];
		}
		return value;
	}
#pragma endregion
...
#pragma region Iterators
	...
	{
		foreach_element(
		    storage_.lines_.cbegin(), storage_.lines_.cend(), fn, 0);
	}
	...
	{
		foreach_element(
		    storage_.lines_.begin(), storage_.lines_.end(), fn, 0);
	}
...
#pragma region Debug
	...
	{
		for (size_type row_index = 0; row_index < num_rows();
		     ++row_index) {
