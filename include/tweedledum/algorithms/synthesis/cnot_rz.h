...
public:
	cnot_rz_encoder(Matrix const& transform,
	    ParityMap<uint32_t> const& parities, Cnf& cnf_builder,
	    cnot_rz_params params)
	    : transform_(transform), parities_(parities),
	      use_symmetry_break_(params.use_symmetry_break),
	      num_terms_(parities.num_terms()), num_moments_(0),
	...
	void encode()
	{
		...
		cnf_builder_.add_variables(
		    transform_.num_rows() * transform_.num_columns());
		// Encode intial states
		for (uint32_t row = 0u; row < transform_.num_rows(); ++row) {
			for (uint32_t column = 0u;
			     column < transform_.num_columns(); ++column) {
				const auto polarity
				    = (row == column) ?
				          bill::positive_polarity :
				          bill::negative_polarity;
				lit_type lit(
				    matrix_var(0, row, column), polarity);
	...
	std::vector<lit_type> encode_assumptions() const
	{
		...
		for (uint32_t row = 0u; row < num_qubits(); ++row) {
			for (uint32_t column = 0u; column < num_qubits();
			     ++column) {
				const auto polarity
				    = transform_.at(row, column) ?
				          bill::positive_polarity :
				          bill::negative_polarity;
				assumptions.emplace_back(
				    matrix_var(num_moments_ - 1, row, column),
				    polarity);
			}
		}
		for (uint32_t term_id = 0u; term_id < num_terms(); ++term_id) {
			assumptions.emplace_back(
			    parity_term_var(num_moments_ - 1, term_id),
			    bill::positive_polarity);
		}
		return assumptions;
	}
	    ...
	    std::vector<lbool_type> const& model)
	{
		...
		for (uint32_t i = 0u; i < num_qubits(); ++i) {
			...
			if (rotation != 0.0) {
				circuit.create_op(
				    GateLib::r1(rotation), qubits.at(i));
			}
		}
		for (uint32_t moment = 0u; moment < (num_moments_ - 1);
		     ++moment) {
			uint32_t control = 0u;
			uint32_t target = 0u;
			for (uint32_t row = 0u; row < num_qubits(); ++row) {
				if (model.at(control_var(moment, row))
				    == lbool_type::true_) {
					control = row;
					assert(model.at(target_var(moment, row))
					       == lbool_type::false_);
				} else if (model.at(target_var(moment, row))
				           == lbool_type::true_) {
					target = row;
					assert(model.at(control_var(moment, row))
					       == lbool_type::false_);
				}
			}
			circuit.create_op(
			    GateLib::cx, qubits.at(control), qubits.at(target));
			qubits_states[target] ^= qubits_states[control];
			Angle rotation
			    = parities_.extract_term(qubits_states[target]);
			if (rotation != 0.0) {
				circuit.create_op(
				    GateLib::r1(rotation), qubits.at(target));
			}
		}
	}
...
private:
	void encode_parity_terms(uint32_t moment)
	{
		...
		cnf_builder_.add_variables(
		    num_terms() + (num_terms() * num_qubits()));
		uint32_t term_id = 0u;
		for (auto const& [term, _] : parities_) {
			(void) _;
			for (uint32_t row = 0u; row < num_qubits(); ++row) {
				for (uint32_t column = 0u;
				     column < num_qubits(); ++column) {
					const auto polarity
					    = (term >> column) & 1 ?
					          bill::positive_polarity :
					          bill::negative_polarity;
					matrix_lits.emplace_back(
					    matrix_var(moment, row, column),
					    polarity);
				}
				encode_and(matrix_lits,
			...
			if (moment > 0) {
				...
				term_lits.emplace_back(
				    parity_term_var(moment - 1, term_id),
				    bill::positive_polarity);
				clause.emplace_back(
				    parity_term_var(moment - 1, term_id),
				    bill::negative_polarity);
				clause.emplace_back(
				    parity_term_var(moment, term_id),
				    bill::positive_polarity);
	...
	void encode_transition(uint32_t moment)
	{
		...
		for (uint32_t row = 0u; row < num_qubits(); ++row) {
			for (uint32_t column = 0u; column < num_qubits();
			     ++column) {
				clause.emplace_back(target_var(moment - 1, row),
				    bill::positive_polarity);
				clause.emplace_back(
				    matrix_var(moment - 1, row, column),
				    bill::negative_polarity);
				clause.emplace_back(
				    matrix_var(moment, row, column),
				    bill::positive_polarity);
				...
				clause.at(1) = lit_type(
				    matrix_var(moment - 1, row, column),
				    bill::positive_polarity);
				clause.at(2)
				    = lit_type(matrix_var(moment, row, column),
				        bill::negative_polarity);
				cnf_builder_.add_clause(clause);
	...
	{
		for (uint32_t other_row = 0u; other_row < num_qubits();
		     ++other_row) {
			if (other_row == row) {
				continue;
			}
			...
			clause.emplace_back(target_var(moment - 1, row),
			    bill::negative_polarity);
			clause.emplace_back(control_var(moment - 1, other_row),
			    bill::negative_polarity);

			clause.emplace_back(matrix_var(moment - 1, row, column),
			    bill::negative_polarity);
			clause.emplace_back(
			    matrix_var(moment - 1, other_row, column),
			    bill::negative_polarity);
			clause.emplace_back(matrix_var(moment, row, column),
			    bill::negative_polarity);
			cnf_builder_.add_clause(clause);
	...
	void symmetry_break_transition(uint32_t moment)
	{
		...
		for (uint32_t control0 = 0u; control0 < num_qubits();
		     ++control0) {
			for (uint32_t target0 = 1u; target0 < num_qubits();
			     ++target0) {
				if (control0 == target0) {
					continue;
				}
				for (uint32_t target1 = 0u; target1 < target0;
				     ++target1) {
					if (control0 == target1) {
						continue;
					}
					clause.emplace_back(
					    control_var(moment - 1, control0),
					    bill::negative_polarity);
					clause.emplace_back(
					    control_var(moment, control0),
					    bill::negative_polarity);
					clause.emplace_back(
					    target_var(moment - 1, target0),
					    bill::negative_polarity);
					clause.emplace_back(
					    target_var(moment, target1),
					    bill::negative_polarity);
					cnf_builder_.add_clause(clause);
					clause.clear();
				}
...
private:
	void encode_and(std::vector<lit_type> const& lits, var_type output)
	{
		...
		std::vector<lit_type> clause_1(
		    2, lit_type(output, bill::negative_polarity));
	...
	void encode_or(std::vector<lit_type> const& lits, var_type output)
	{
		std::vector<lit_type> clause_0;
		std::vector<lit_type> clause_1(
		    2, lit_type(output, bill::positive_polarity));
		for (lit_type lit : lits) {
			clause_1.at(0) = ~lit;
			cnf_builder_.add_clause(clause_1);
...
private:
	...
	{
		var_type var
		    = (moment * offset_) + (row * num_qubits()) + column;
	...
	{
		var_type var
		    = (moment * offset_) + (num_qubits() * num_qubits()) + id;
		assert(var < var_type(cnf_builder_.num_variables()));
		return var;
	}

	var_type parity_term_row_var(
	    uint32_t moment, uint32_t id, uint32_t row) const
	{
		var_type var = (moment * offset_)
		               + (num_qubits() * num_qubits()) + num_terms()
		               + (id * num_qubits()) + row;
		assert(var < var_type(cnf_builder_.num_variables()));
		return var;
	}
	...
	{
		var_type var = moment * offset_ + (num_qubits() * num_qubits())
		               + num_terms() + (num_terms() * num_qubits())
		               + row;
		assert(var < var_type(cnf_builder_.num_variables()));
		return var;
...
{
	BitMatrixRM<> matrix(qubits.size(), qubits.size());
