...
struct tbs_params {
	using cost_fn_type = std::function<uint32_t(
	    std::vector<uint32_t> const&, uint32_t, uint32_t)>;
	...
	enum class behavior : uint8_t {
		 ...
		 */
		unidirectional,
		/*! \brief adds gates from input **or** output side at each
		   step. */
		bidirectional,
		/*! \brief adds gates from input **and** output side at each
		step \verbatim embed:rst (for more information see
		:cite:`SDRM16`) \endverbatim */
		multidirectional,
	};
...
inline std::vector<wire::Id> to_qubit_vector(
    IntType bits, std::vector<wire::Id> const& qubits)
{
...
inline void update_permutation(
    std::vector<uint32_t>& permutation, uint32_t controls, uint32_t targets)
{
    ...
    std::vector<uint32_t>& permutation)
{
	...
	for (auto const& [controls, targets] : gates) {
		circuit.create_op(GateLib::ncx,
		    detail::to_qubit_vector(controls, qubits),
 ...
 *
 * This is the in-place variant of ``tbs``, in which the circuit is passed as a
 * parameter and can potentially already contain some gates. The parameter
 * ``qubits`` provides a qubit mapping to the existing qubits in the circuit.
 *
 ...
 */
template<typename Circuit>
void tbs(Circuit& circuit, std::vector<wire::Id> const& qubits,
    std::vector<uint32_t> permutation, tbs_params params = {})
{
	...
	case tbs_params::behavior::multidirectional:
		detail::tbs_multidirectional(
		    circuit, qubits, permutation, params);
		break;
	}
}
