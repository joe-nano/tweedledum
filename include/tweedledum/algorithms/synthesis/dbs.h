...
inline auto decompose(std::vector<uint32_t>& permutation, uint8_t var)
{
	...
	while (true) {
		if (visited[row]) {
			const auto it
			    = std::find(visited.begin(), visited.end(), 0);
			if (it == visited.end()) {
				break;
			}
...
inline auto control_function_abs(
    uint32_t num_vars, std::vector<uint32_t> const& permutation)
{
    ...
    dbs_params params = {})
{
	...
	std::list<std::pair<kitty::dynamic_truth_table, std::vector<wire::Id>>>
	    gates;
