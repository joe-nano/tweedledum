...
public:
	tk_swap_cnf_encoder(Device const& graph,
	    std::vector<uint32_t> const& init_cfg,
	    std::vector<uint32_t> const& final_cfg, Cnf& cnf_builder,
	    swap_network_params::opt_goals goal)
	    : graph_(graph), init_cfg_(init_cfg), init_t2v_(init_cfg.size(), 0),
	      final_cfg_(final_cfg), is_bullshit_(), num_moments_(0),
	      offset_(num_vertices() * num_vertices() + num_edges()),
	      cnf_builder_(cnf_builder), vertice_egdes_map_(num_vertices())
	{
	...
	void encode()
	{
		...
		for (uint32_t i = 0; i < init_cfg_.size(); ++i) {
			lit_type lit(token_vertice_var(0, init_cfg_[i], i),
	...
	std::vector<lit_type> encode_assumptions()
	{
		...
		// Assume intial configuration
		// for (uint32_t i = 0; i < init_cfg_.size(); ++i) {
		// 	sat::var_type var = token_vertice_var(0, init_cfg_[i],
		// i); 	assumptions.emplace_back(var, sat::positive_polarity);
		// }
		// Assume final configuration
		for (uint32_t i = 0; i < final_cfg_.size(); ++i) {
			var_type var = token_vertice_var(
			    num_moments_ - 1, final_cfg_[i], i);
	...
	void pre_process()
	{
		...
		for (uint32_t k = 0; k < init_cfg_.size(); ++k) {
			if (init_cfg_[k] != final_cfg_[k]) {
				auto it = std::find(final_cfg_.begin(),
				    final_cfg_.end(), init_cfg_[k]);
				uint32_t idx
				    = std::distance(final_cfg_.begin(), it);
				uint32_t dist = graph_.distance(k, idx);
				sum_distance += dist;
				max_distance = std::max(max_distance, dist);
			}
		}
		// When optimizing for swap number, each moment hold only one
		// SWAP
		if (opt_num_swaps_) {
			...
			// If the minimum number of moments is odd, but the sgn
			// of the permutations is the same, we add one, as we
			// know that the solution must have an even number of
			// swaps.
			//
			// If the minimum number of moments is even, but the sgn
			// of the permutations are different, then we also need
			// to add one.
...
private:
	var_type token_vertice_var(
	    uint32_t moment, uint32_t token, uint32_t vertice)
	{
	...
	{
		return moment * offset_ + num_vertices() * num_vertices()
		       + edge;
	}
...
private:
	void create_token_vertice_variables()
	{
		...
		for (uint32_t token = 0; token < num_vertices(); ++token) {
			for (uint32_t vertice = 0; vertice < num_vertices();
			     ++vertice) {
				if (graph_.distance(vertice, init_t2v_[token])
				    <= (num_moments_ + 1)) {
					variables.emplace_back(
					    cnf_builder_.add_variable());
					is_bullshit_.emplace_back(0);
					continue;
				}
				cnf_builder_.add_clause(
				    lit_type(cnf_builder_.add_variable(),
		...
		{
			for (uint32_t token = 0; token < num_vertices();
			     ++token) {
				if (graph_.distance(vertice, init_t2v_[token])
				    <= (num_moments_ + 1)) {
					variables.emplace_back(token_vertice_var(
					    num_moments_, token, vertice));
				}
			}
			bill::at_least_one(variables, cnf_builder_);
			bill::at_most_one_pairwise(variables, cnf_builder_);
	...
	void add_moment()
	{
		...
		for (uint32_t vertice = 0; vertice < num_vertices(); ++vertice)
		{
			for (uint32_t token = 0; token < num_vertices();
			     ++token) {
				var_type prev_var = token_vertice_var(
				    num_moments_ - 1, token, vertice);
				var_type current_var = token_vertice_var(
				    num_moments_, token, vertice);
				if (is_bullshit_[current_var])
					continue;
				for (uint32_t edge :
				    vertice_egdes_map_[vertice]) {
					var_type edge_var
					    = swap_var(num_moments_ - 1, edge);
					clause.clear();
					clause.emplace_back(current_var,
					    bill::negative_polarity);
					clause.emplace_back(
					    prev_var, bill::negative_polarity);
					clause.emplace_back(
					    edge_var, bill::negative_polarity);
					cnf_builder_.add_clause(clause);
				}
			}
		}
		// *) Condition 2:
		for (uint32_t vertice = 0; vertice < graph_.num_qubits();
		     ++vertice) {
			for (uint32_t token = 0; token < graph_.num_qubits();
			     ++token) {
				var_type prev_var = token_vertice_var(
				    num_moments_ - 1, token, vertice);
				var_type current_var = token_vertice_var(
				...
				for (uint32_t edge :
				    vertice_egdes_map_[vertice]) {
					edge_vars.emplace_back(
					    swap_var(num_moments_ - 1, edge));
					edge_lits.emplace_back(edge_vars.back(),
					    bill::positive_polarity);
					auto [u, v] = graph_.edge(edge);
					if (u != vertice) {
						token_lits.emplace_back(
						    token_vertice_var(
						        num_moments_ - 1, token,
						        u),
						    bill::positive_polarity);
					} else {
						token_lits.emplace_back(
				...
				if (!opt_num_swaps_) {
					at_most_one_pairwise(
					    edge_vars, cnf_builder_);
				}
				if (is_bullshit_[current_var])
					continue;
				clause = edge_lits;
				clause.emplace_back(
				    current_var, bill::negative_polarity);
				clause.emplace_back(
				    prev_var, bill::positive_polarity);
				assert(edge_lits.size() == token_lits.size());
				for (uint32_t i = 0;
				     i < (1u << edge_lits.size()); ++i) {
					for (uint32_t j = i, k = 0; j;
					     j >>= 1, ++k) {
						if (j & 1) {
							clause[k]
							    = token_lits[k];
	...
	void symmetry_break(uint32_t prev_moment, uint32_t current_moment)
	{
		...
		for (uint32_t i = 0; i < graph_.num_edges(); ++i) {
			...
			for (uint32_t j = i + 1; j < graph_.num_edges(); ++j) {
				...
				if (u_i == u_j || u_i == v_j || v_i == u_j
				    || v_i == v_j) {
					continue;
				}
				clause.emplace_back(swap_var(prev_moment, j),
				    bill::negative_polarity);
				clause.emplace_back(swap_var(current_moment, j),
				    bill::negative_polarity);
				cnf_builder_.add_clause(clause);
				clause.clear();
			}
...
{
	bill::solver solver;
	tk_swap_cnf_encoder encoder(
	    device, init_cfg, final_cfg, solver, params.opt_goal);
	encoder.encode();
	do {
