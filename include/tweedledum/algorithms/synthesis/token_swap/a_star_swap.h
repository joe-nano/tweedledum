...
private:
	...
	struct node_type {
		...
		node_type(map_type const& mapping_, uint32_t edge,
		    uint32_t prev, uint32_t g_, uint32_t h_)
		    : mapping(mapping_), swap(edge), previous(prev), g(g_),
		      h(h_), closed(false)
		{}
	};

public:
	explicit a_star_swapper(Device const& topology) : topology_(topology) {}

	std::vector<swap_type> run(map_type const& init_mapping,
	    map_type const& final_mapping, bool admissable = true)
	{
		...
		while (!open_nodes.empty()) {
			...
			for (uint32_t i = 0; i < topology_.num_edges(); ++i) {
				...
				std::swap(
				    new_mapping.at(topology_.edge(i).first),
				    new_mapping.at(topology_.edge(i).second));

				// Try to add new mapping to the mappings
				// database
				auto [it, was_added] = mappings.emplace(
				    new_mapping, nodes.size());
				node_type& new_node
				    = was_added ? nodes.emplace_back(
				          new_mapping, i, closed_nodes.back(),
				          node.g + 1, 0) :
				                  nodes.at(it->second);

				if (was_added) {
					// If a new node was created, need to
					// add to the list of nodes
					open_nodes.push_back(nodes.size() - 1);
				} else if (new_node.g <= node.g + 1) {
					// Do not update a node if its new cost
					// exeeds the previous one
					continue;
				} else if (new_node.closed) {
					// If new cost is smaller and the node
					// was already closed, re-open it!
					new_node = nodes.emplace_back(
					    new_mapping, i, closed_nodes.back(),
					    node.g + 1, 0);
					open_nodes.push_back(nodes.size() - 1);
					mappings.at(new_mapping)
					    = nodes.size() - 1;
				}
				for (auto k = 0ull; k < final_mapping.size();
				     ++k) {
				...
				if (admissable) {
					new_node.h
					    = std::ceil(new_node.h / 2.0);
				}
			}
			auto min_it = std::min_element(open_nodes.begin(),
			    open_nodes.end(), [&](auto a_idx, auto b_idx) {
				    auto& a = nodes[a_idx];
				    auto& b = nodes[b_idx];
...
auto a_star_swap(Device const& topology,
    std::vector<uint32_t> const& init_mapping,
