...
public:
	a_star_mapper(device const& device, a_star_map_params const& params)
	    : device_(device), distances_(device.distance_matrix()),
	...
	MappedDAG run(
	    Network const& original, std::vector<uint32_t> initial_mapping = {})
	{
		MappedDAG mapped(original, device_);
		LayersView<Network> layered(original);

		// Define mapping first layer
		// TODO: make this better
		if (initial_mapping.empty()) {
			initial_mapping
			    = compute_initial_mapping(layered, mapped);
		}
	...
	//
private:
	bool execute_layer(Network const& layered, uint32_t layer,
	    MappedDAG& mapped, std::set<wire_id>& v_qubits)
	{
		...
		for (node_id const n_id : nodes) {
			...
			} else if (mapped.add_op(op, op.control(), op.target())
			           == node::invalid_id) {
...
private:
	struct node {
		...
		node(std::vector<wire_id> const& mapping_, uint32_t edge,
		    uint32_t prev, uint32_t g, uint32_t h)
		    : mapping(mapping_), swap(edge), previous(prev), g(g), h(h),
		      closed(false)
		{}
	};

	void fix_layer(LayersView<Network> const& layered, MappedDAG& mapped,
...
fix_layer_again:
		...
		while (!open_nodes.empty()) {
			...
			for (uint32_t i = 0u; i < device_.edges.size(); ++i) {
				// Do not apply the same SWAP gate twice in a
				// row if (i == node.swap) { 	continue;
				// }
				auto [v, u] = device_.edges(i);
				auto search_v
				    = v_qubits.find(node.mapping.at(v));
				auto search_u
				    = v_qubits.find(node.mapping.at(u));
				// Apply only SWAP operations including at least
				// one qubit in used_qubits
				if ((search_v == v_qubits.end())
				    && (search_u == v_qubits.end())) {
					continue;
				}
				...
				auto [map_it, was_added] = mappings.emplace(
				    new_mapping, nodes.size());
				auto& new_node = was_added ?
				                     nodes.emplace_back(node) :
				                     nodes.at(map_it->second);
				if (was_added) {
					open_nodes.push_back(nodes.size() - 1);
					std::swap(new_node.mapping[v],
					    new_node.mapping[u]);
				...
				// Check wheter a goal state is reached (i.e.
				// whether any gate can be applied) and
				// determine heuristic cost
				auto min_it = open_nodes.end();
				for (node_id const n_id : unexecuted_nodes_) {
					op_type const& op
					    = layered_network.node(n_id).op;
					auto [q0, q1]
					    = find_qubits(node.mapping,
				...
				if (min_it != open_nodes.end()) {
					closed_nodes.push_back(
					    open_nodes.back());
					goto found_goal;
				}
				min_it = std::min_element(open_nodes.begin(),
				    open_nodes.end(),
...
found_goal:
		...
		for (auto node_index : layer) {
			...
			if (!mapped_network.add_gate(
			        gate, gate.control(), gate.target())) {
				tmp_layer.push_back(node_index);
				tmp_virtual_qubits.emplace(
				    mapped_network.virtual_qubit(
				        gate.control()));
				tmp_virtual_qubits.emplace(
	...
	std::pair<uint32_t, uint32_t> find_qubits(
	    std::vector<uint32_t> const& mapping, uint32_t q0, uint32_t q1)
	{
	...
	std::vector<uint32_t> compute_initial_mapping(
	    LayersView<Network> const& layered_network,
	    mapping_view<Network> const& mapped_network)
	{
		...
		std::iota(
		    unused_phy_qubits.begin(), unused_phy_qubits.end(), 0u);
		std::vector<uint32_t> layer = layered_network.layer(1);
		auto max = std::max_element(swap_layers_.begin(),
		    swap_layers_.end(), [](auto const& a, auto const& b) {
		...
		for (auto node_index : layer) {
			...
			fmt::print("{}, {}\n", phy_q0, phy_q1);
			mapping.at(mapped_network.virtual_qubit(gate.control()))
			    = phy_q0;
			mapping.at(mapped_network.virtual_qubit(gate.target()))
			    = phy_q1;
			unused_phy_qubits.erase(
			    std::remove_if(unused_phy_qubits.begin(),
			    ...
			    unused_phy_qubits.end());
			// fmt::print("Unused: {}\n",
			// fmt::join(unused_phy_qubits.begin(),
			// unused_phy_qubits.end(), ","));
			edges.pop_back();
		}
	...
	void compute_swap_layers()
	{
		...
		for (auto& i : incidents) {
			...
			for (auto o : i) {
				set = zdd_swap_layers.union_(
				    set, zdd_swap_layers.elementary(o));
			}
		...
		auto layers
		    = zdd_swap_layers.nonsupersets(univ_fam, edges_union);
