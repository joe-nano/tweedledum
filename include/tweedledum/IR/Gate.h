...
// I do not want to have different classes for the different gate categories
// (Meta, non-parameterisable, and parameterisable), so I will define one that
...
public:
	explicit Gate(gate_ids id, Angle const& theta, Angle const& phi,
	    Angle const& lambda)
	    : id_(id),
	      id_adjoint_(detail::gate_info[static_cast<uint32_t>(id)].adjoint),
	      axis_(detail::gate_info[static_cast<uint32_t>(id)].axis),
	      info_bits_(resolve_info_bits(id)), theta_(theta), phi_(phi),
	      lambda_(lambda)
	{
		assert((info_bits_ & is_parameterisable_)
		       && "You should NOT be using this!");
		(void) pad_;
	}

	// I want to differentiate between the constexpr constructor and the
	// non-constexpr, but they take the same arguments.  So to change the
	// signature I inverted the order of the arguments ^-^
	//
	// Users should not use this constructor!
	constexpr Gate(Angle const& theta, Angle const& phi,
	    Angle const& lambda, gate_ids id)
	    : id_(id),
	      id_adjoint_(detail::gate_info[static_cast<uint32_t>(id)].adjoint),
	      axis_(detail::gate_info[static_cast<uint32_t>(id)].axis),
	      info_bits_(resolve_info_bits(id)), theta_(theta), phi_(phi),
...
#pragma region Properties
	...
	constexpr bool is_adjoint(Gate const& other) const
	{
		...
		// For the parameterisables gates, I need to make sure that the
		// angles cancel out:
		switch (id()) {
		case gate_ids::r1:
		case gate_ids::rx:
		...
		case gate_ids::ncry:
		case gate_ids::ncrz:
			if (rotation_angle() + other.rotation_angle()
			    != sym_angle::zero) {
				return false;
			}
			return true;
...
#pragma region Parameters
	...
	{
		assert(!is_meta() && !is_measurement() && !is(gate_ids::swap)
		       && !is(gate_ids::u3));
		if (is_r1()) {
			return lambda_;
		}
...
private:
	// A lean ``gate`` object would take only the identifier and three angle
	// parameters. However, the alignment would be off, so the compiler will
	// align by padding it (add bytes) Well, rather than letting this
	// padding happen, which effectively waste space, let's add it
	// ourselves, and put this space to good use:
	//     * I will have the gate to carry both its ``id`` and it's adjoint
	//     gate ``id``. This
	//       can simplify testing whether two gates are adjoint or not.
	//
	//     * I wil also add an hand implemented bit field, `info_bits_`, to
	//     store some useful
	//       information about the gate. The names are pretty much
	//       self-explanatory:
	enum : uint32_t {
		is_meta_ = (1u << 0),
	...
	//
	// FIXME: I will leave this tag here because I see that on the long term
	// this decision will come to bite me ^-^
...
// Measurement gates
constexpr Gate measure_x(
    sym_angle::zero, sym_angle::zero, sym_angle::zero, gate_ids::measure_x);
constexpr Gate measure_y(
    sym_angle::zero, sym_angle::zero, sym_angle::zero, gate_ids::measure_y);
constexpr Gate measure_z(
    sym_angle::zero, sym_angle::zero, sym_angle::zero, gate_ids::measure_z);

// One-qubit gates
constexpr Gate i(sym_angle::zero, sym_angle::zero, sym_angle::zero, gate_ids::i);
constexpr Gate h(
    sym_angle::pi_half, sym_angle::zero, sym_angle::pi, gate_ids::h);
constexpr Gate x(sym_angle::pi, sym_angle::zero, sym_angle::pi, gate_ids::x);
constexpr Gate y(
    sym_angle::pi, sym_angle::pi_half, sym_angle::pi_half, gate_ids::y);
constexpr Gate z(sym_angle::zero, sym_angle::zero, sym_angle::pi, gate_ids::z);
constexpr Gate s(
    sym_angle::zero, sym_angle::zero, sym_angle::pi_half, gate_ids::s);
constexpr Gate sdg(
    sym_angle::zero, sym_angle::zero, -sym_angle::pi_half, gate_ids::sdg);
constexpr Gate t(
    sym_angle::zero, sym_angle::zero, sym_angle::pi_quarter, gate_ids::t);
constexpr Gate tdg(
    sym_angle::zero, sym_angle::zero, -sym_angle::pi_quarter, gate_ids::tdg);

// Two-qubit unitary gates
constexpr Gate cx(sym_angle::pi, sym_angle::zero, sym_angle::pi, gate_ids::cx);
constexpr Gate cy(
    sym_angle::pi, sym_angle::pi_half, sym_angle::pi_half, gate_ids::cy);
constexpr Gate cz(sym_angle::zero, sym_angle::zero, sym_angle::pi, gate_ids::cz);
constexpr Gate swap(
    sym_angle::zero, sym_angle::zero, sym_angle::zero, gate_ids::swap);

// N-qubit unitary gates
constexpr Gate ncx(sym_angle::pi, sym_angle::zero, sym_angle::pi, gate_ids::ncx);
constexpr Gate ncy(
    sym_angle::pi, sym_angle::pi_half, sym_angle::pi_half, gate_ids::ncy);
constexpr Gate ncz(
    sym_angle::zero, sym_angle::zero, sym_angle::pi, gate_ids::ncz);
...
{
	return Gate(
	    gate_ids::rx, theta, -sym_angle::pi_half, sym_angle::pi_half);
}
...
{
	return Gate(
	    gate_ids::crx, theta, -sym_angle::pi_half, sym_angle::pi_half);
}
...
{
	return Gate(
	    gate_ids::ncrx, theta, -sym_angle::pi_half, sym_angle::pi_half);
}
...
constexpr uint64_t classic_rev
    = create(gate_ids::x, gate_ids::cx, gate_ids::ncx, gate_ids::swap);

constexpr uint64_t cnot_rz
    = create(gate_ids::x, gate_ids::z, gate_ids::s, gate_ids::t, gate_ids::sdg,
        gate_ids::tdg, gate_ids::cx, gate_ids::r1, gate_ids::rz);

constexpr uint64_t clifford_t = create(gate_ids::i, gate_ids::h, gate_ids::x,
    gate_ids::y, gate_ids::z, gate_ids::s, gate_ids::t, gate_ids::sdg,
    gate_ids::tdg, gate_ids::cx, gate_ids::cy, gate_ids::cz, gate_ids::swap);
