...
#pragma region Constructors
	wn32_op(Gate const& g, wire::Id const t)
	    : Gate(g), is_qubit_(t.is_qubit() << t), polarity_(0), controls_(0),
	      targets_(1u << t)
	{
	...
	wn32_op(Gate const& g, wire::Id const w0, wire::Id const w1)
	    : Gate(g), is_qubit_((w0.is_qubit() << w0) | (w1.is_qubit() << w1)),
	      polarity_(w0.is_complemented() << w0), controls_(1u << w0),
	      targets_(1u << w1)
	{
	...
	wn32_op(Gate const& g, wire::Id const c0, wire::Id const c1,
	    wire::Id const t)
	    : Gate(g), is_qubit_((c0.is_qubit() << c0) | (c1.is_qubit() << c1)
	                         | (t.is_qubit() << t)),
	      polarity_(
	          (c0.is_complemented() << c0) | (c1.is_complemented() << c1)),
	      controls_((1u << c0) | (1u << c1)), targets_(1u << t)
	{
		assert(c0 != wire::invalid_id);
		assert(c1 != wire::invalid_id);
		assert(t != wire::invalid_id);
		assert(c0.uid() != c1.uid() && c0.uid() != t.uid()
		       && c1.uid() != t.uid());
		assert(!is_meta() && !is_measurement());
		assert(!is_one_qubit() && !is_two_qubit());
	}

	wn32_op(Gate const& g, std::vector<wire::Id> const& cs,
	    std::vector<wire::Id> const& ts)
	    : Gate(g), is_qubit_(0), polarity_(0), controls_(0), targets_(0)
	{
		assert(!ts.empty());
...
#pragma region Properties
	...
	wire::Id control(uint32_t const i = 0u) const
	{
		...
		return wire::Id(
		    idx, (is_qubit_ >> idx) & 1, (polarity_ >> idx) & 1);
	}
	...
	bool is_dependent(wn32_op const& other) const
	{
		...
		if (((controls_ | targets_) & (other.controls_ | other.targets_))
		    == 0u) {
			return false;
		}
		...
		if ((ct | tc) == 0u) {
			// If have at least one target in common, and their
			// rotation axis are different, then they are dependent:
...
private:
	/*! \brief bitmap which indicates which wire in the network are the
	 * qubits. */
	uint32_t is_qubit_;
