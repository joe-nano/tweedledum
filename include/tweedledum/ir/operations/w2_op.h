...
#pragma region Constructors
	...
	w2_op(Gate const& g, wire::Id const c0, wire::Id const c1,
	    wire::Id const t)
	    : Gate(g), wires_({wire::invalid_id, wire::invalid_id})
	{
		(void) c0;
		(void) c1;
		(void) t;
	...
	w2_op(Gate const& g, std::vector<wire::Id> const& cs,
	...
	bool is_dependent(w2_op const& other) const
	{
		...
		if (is_two_qubit() || other.is_two_qubit()) {
			...
			if (wires_.at(0u).uid() == other.wires_.at(0u).uid()) {
				wires_intersec = true;
			} else if (wires_.at(0u).uid()
			           == other.wires_.at(1u).uid()) {
				wires_intersec = true;
			} else if (wires_.at(1u).uid()
			           == other.wires_.at(0u).uid()) {
				wires_intersec = true;
			} else if (wires_.at(1u).uid()
			           == other.wires_.at(1u).uid()) {
				wires_intersec = true;
			...
			// Both have one wire
			// If targets are the same, then I just need to worry
			// about the axis
