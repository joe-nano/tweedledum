...
/*! \brief
 *
 * lexer class provides a simple interface that turns a text buffer into a stream of tokens.  This
 * provides no support for file reading or buffering, or buffering/seeking of tokens, only forward
 * lexing is supported.
 *
 * The lexer does't return tokens for every character in the file, it skips whitespace and comments.
 */
class Lexer {
public:
	// Create a new lexer object for the specified source. This lexer assumes that the
	// associated file buffer will outlive it, so it doesn't take ownership of it,
	// hence 'string_view'.
	Lexer(Source const* source) : source_(source) , src_position_(source_->cbegin())
	{}
...
private:
	...
	// Skip over a series of whitespace characters.  Update `src_position_` to point to the next
	...
	// We have just read the `//` characters from input. Skip until we find the newline or EOF
	// character thats terminate the comment.  Then update `src_position_` and return.
	...
	// When we lex a identifier or a numeric constant token, the token is formed by a span of
	// chars starting at buffer_prt and going till token_end.  This method takes that range and
	// assigns it to the token as its location and size.  It also update `src_position_`.
	...
	{
		while (std::isalpha(*current_pos_) || std::isdigit(*current_pos_) || *current_pos_ == '_') {
			++current_pos_;
		}
	...
	// Return the next token in the buffer. If this is the end of buffer, it return the EOF token.
	Token lex()
	{
	lex_next_token:
	...
	// Current pointer into source content. (Next character to be lexed)
